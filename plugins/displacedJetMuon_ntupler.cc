...
void displacedJetMuon_ntupler::enableGenParticleBranches()
{
  ...
  displacedJetMuonTree->Branch("gParticleDecayVertexZ", gParticleDecayVertexZ, "gParticleDecayVertexZ[nGenParticle]/F");
   ...
   displacedJetMuonTree->Branch("gLLP_id", gLLP_id, "gLLP_id[2]/I");
...
bool displacedJetMuon_ntupler::fillGenParticles(){
  ...
  for(size_t i=0; i<genParticles->size();i++)
  {
   ...
   //     || (abs((*genParticles)[i].pdgId()) == 9900012 || abs((*genParticles)[i].pdgId()) == 9900014 
   // 	   || abs((*genParticles)[i].pdgId()) == 9900016)
   //     )
   //     {
  ...
  for(unsigned int i = 0; i < prunedV.size(); i++) {
	...
	cout << "matchedLLP ID: " << matchedLLPID << "\n";

	//find the last daughter with same ID
	while (tmpParticle->numberOfDaughters() == 1 && tmpParticle->pdgId() == tmpParticle->daughter(0)->pdgId()) {
	  cout << "Going down daughter chain -> " << tmpParticle->daughter(0)->pdgId() << " " << tmpParticle->daughter(0)->status() << "\n";
	  tmpParticle = tmpParticle->daughter(0);
	}

	cout << "Final LLP found: " << tmpParticle->pdgId() << " " << tmpParticle->status() << "\n";
	...
	cout << "NDaughter:" << tmpParticle->numberOfDaughters() << "\n";
	cout << "found Daughter: " << foundDaughter << "\n";
	  ...
	  cout << "Decay: " << dau->vx() << " " << dau->vy() << " " << dau->vz() << " \n"; 
...
bool displacedJetMuon_ntupler::fillJets(const edm::EventSetup& iSetup)
{
  ...
  for (unsigned int iTrack = 0; iTrack < generalTrackHandle->size(); iTrack ++){
    ...
    if (SaveThisTrack[iTrack] || tref->pt() > 30) {
